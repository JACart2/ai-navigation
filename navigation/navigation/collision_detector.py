...
class CollisionDetector(object):
    ...
    def __init__(self):
        ...
        self.front_axle_track = rospy.get_param("front_axle_track", 0.9017)
        self.rear_axle_track = rospy.get_param("rear_axle_track", 0.9652)
        self.tire_width = rospy.get_param("tire_width", 0.2159)
        self.cur_speed = 0.1
        self.stopped = False
        ...
        self.safe_obstacle_dist = rospy.get_param(
            "safe_obstacle_dist", 6 * factor
        )  # inital 6
        self.safe_obstacle_time = rospy.get_param(
            "safe_obstacle_time", 2 * factor
        )  # inital 2

        self.obstacle_sub = rospy.Subscriber(
            "/obstacles", ObstacleArray, self.obstacle_callback, queue_size=10
        )
        self.pos_sub = rospy.Subscriber(
            "/ndt_pose", PoseStamped, self.position_callback, queue_size=10
        )
        self.angle_sub = rospy.Subscriber(
            "/nav_cmd", VelAngle, self.angle_callback, queue_size=10
        )
        self.cur_speed_sub = rospy.Subscriber(
            "/estimated_vel_mps", Float32, queue_size=10
        )
        ...
        self.display_boundary_pub = rospy.Publisher(
            "/boundaries", Marker, queue_size=10
        )
        self.display_array = rospy.Publisher(
            "/boundaries_array", MarkerArray, queue_size=100
        )
        self.collision_pub = rospy.Publisher(
            "/collision_pub", MarkerArray, queue_size=100
        )
            ...
            if potential_collision:
                ...
                if distance < (self.safe_obstacle_dist / 3) or impact_time < (
                    ...
                    if not self.stopped:
                        ...
                        rospy.logwarn(
                            "Requesting a fast stop due to possible collision"
                        )
