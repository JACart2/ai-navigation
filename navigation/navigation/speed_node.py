...
import rclpy
from geometry_msgs.msg import Pose, Point, PoseStamped, PointStamped, TwistStamped, PoseWithCovarianceStamped
...
class SpeedNode(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        self.twist_pub = self.create_publisher(
            TwistStamped, "/estimate_twist", 10
        )
    ...
    def pose_cb(self, msg):
        ...
        if self.prev_pose != None:
            ...
            distance = math.sqrt((msg.pose.pose.position.y - self.prev_pose.position.y) ** 2 + (msg.pose.pose.position.x - self.prev_pose.position.x)**2)
            delta_time = time.time() - self.prev_time

            #smoothing
            
            self.speed_estimate = self.speed_estimate * .8 + (distance / delta_time) *.2
