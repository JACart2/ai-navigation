...
class LocalPlanner(object):
    def __init__(self):
        ...
        self.vehicle_state_pub = rospy.Publisher(
            "/vehicle_state", VehicleState, queue_size=10, latch=True
        )
    ...
    def create_path(self):
        ...
        if len(ax) > 2:
            ...
            state = State(
                x=pose.position.x, y=pose.position.y, yaw=angles[2], v=initial_v
            )
            ...
            while (
                last_index > target_ind and self.path_valid and not rospy.is_shutdown()
            ):
                target_speed = self.global_speed
    ...
    def calc_eta(self, event):
        ...
        if self.current_state.is_navigating:
            ...
            current_node = self.get_closest_point(
                self.global_pose.position.x, self.global_pose.position.y
            )
    ...
    def calc_trip_dist(self, points_list, start):
        ...
        for i in range(prev_node, len(points_list)):
            trip_sum += self.calc_distance(
                points_list[prev_node].x,
                points_list[prev_node].y,
                points_list[i].x,
                points_list[i].y,
            )
            prev_node = i
